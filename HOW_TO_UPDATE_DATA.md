...
### Update Staff
```bash
curl -X POST https://core-checks-only.preview.emergentagent.com/api/admin/update-staff \
...
### Update Assets
```bash
curl -X POST https://core-checks-only.preview.emergentagent.com/api/admin/update-assets \
  -H "Content-Type: application/json" \
